...
export class SearchPageComponent implements OnInit {
    ...
    private setResults(query: string, results: SearchResult[]) {
        ...
        this.movies$.next(results.filter(r => r.type === MEDIA_TYPE.TITLE && !r.is_series));
        this.series$.next(results.filter(r => r.type === MEDIA_TYPE.TITLE && r.is_series));
        this.people$.next(results.filter(r => r.type === MEDIA_TYPE.PERSON));
    }
}
